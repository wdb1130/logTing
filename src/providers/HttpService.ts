...
export class HttpService {
  ...
  public request(url: string, options: RequestOptionsArgs, isShow?:boolean): Observable<Response> {
    url = HttpService.formatUrl(url);
    this.optionsAddToken(options);
    return Observable.create(observer => {
      if(!isShow){
        this.nativeService.showLoading('玩命加载中...');
      }
  ...
  //专用于上拉加载数据
  public postPage(url: string, paramMap: any = null, isShow:boolean = true): Observable<Response> {
    return this.request(url, new RequestOptions({
      method: RequestMethod.Post,
      body: HttpService.buildURLSearchParams(paramMap).toString(),
      headers: new Headers({
